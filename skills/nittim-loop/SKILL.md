...
Never a cached or remembered copy — this checklist is versioned and can
change, so read it fresh right now:

- If the nittim MCP server is connected, call the `get_loop` tool — free, no
  key needed, no account, nothing sent anywhere. It returns this same
  checklist as its result.
- Otherwise, fetch `https://nittim.com/selfcheck.md` directly.

Read it end to end before touching any code. It defines the 13 review
categories, the output format, and the exact procedure below; what follows
is a summary for this harness, not a replacement for reading it.
...
- `passes` — one entry per pass actually run, in order, each with its
  ...
  count left out defaults to 0), how many findings it fixed, and whether it
